...
public final class ConjuntoAlunos implements IConjunto{
    ...
    public boolean excluirPorMatricula(String matricula) throws Exception {
        try {
            boolean removidoArray = false;
            Iterator<Aluno> iterator = array.iterator();

            while (iterator.hasNext()) {
                Aluno aluno = iterator.next();
                String alunoMatricula = aluno.getMatricula().replaceAll("[^0-9]", "");
            ...
            if (removidoArray) {
                ...
                for (Aluno aluno : array) {
                    arvore.incluir(aluno.getMatricula(), aluno);
                }
            }

            return removidoArray;
        } catch (Exception erro) {
            throw new Exception("Erro ao excluir por matrícula: " + erro.getMessage());
        }
    }
    ...
    public boolean excluirPorNome(String nome) throws Exception {
        try {
            ...
            if (removidoArray) {
                arvore = new Arvore_binaria();
                for (Aluno aluno : array) {
                    arvore.incluir(aluno.getNome(), aluno);
                }
            }

            return removidoArray;
        } catch (Exception erro) {
            throw new Exception("Erro ao excluir por nome: " + erro.getMessage());
        }
    }
